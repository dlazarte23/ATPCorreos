...
export const logearUsuarioAction = (usuario) => {
  return async (dispatch) => {
    ...
    try {
      const endpoint = `${uri.setLoginUser}?shortUser=${usuario.username}&password=${usuario.password}`;

      const response = await get(endpoint);

      if (response.code === 200) {
        message.success(`Bienvenido ${response.user.shortUser} !`);

        dispatch(logearUsuarioExito(response.user));

        localStorage.setItem("IS_AUTHENTICATED", true);
        localStorage.setItem("DATA_SESION", JSON.stringify(response.user));
